...
SELECT AVG(lifeexpectancy) AS 'average_life_expectany', continent
FROM country
WHERE continent = 'South America'
GROUP BY continent
-- 5. The sum of the population of all cities in California.
-- (total population of all cities in California: 16716706)
SELECT SUM(population) AS 'sum_of_population', district
FROM city
...
SELECT TOP 1 (population / surfacearea) AS 'population_density', name
FROM country
...
-- (45 rows)
SELECT COUNT(name) AS 'count_of_cities', district
FROM city
WHERE countrycode = 'USA'
GROUP BY district
ORDER BY district
...
SELECT MAX(population) 'maximum_population', countrycode
FROM city
GROUP BY countrycode
